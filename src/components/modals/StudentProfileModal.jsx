...
const StudentProfileModal = ({ student, grade, subjects, onClose, openModal, testOverrides = {} }) => {
  ...
  const completionRate =
    assignmentStats.totalAssignments > 0
      ? Math.round((assignmentStats.submitted / assignmentStats.totalAssignments) * 100)
      : 0;

  const behaviorStats = React.useMemo(() => {
    const positives = filteredBehaviorLogs.filter((log) => log.type === 'positive').length;
    const negatives = filteredBehaviorLogs.filter((log) => log.type !== 'positive').length;
    const total = filteredBehaviorLogs.length;
                ...
                <div className="flex items-center gap-4 sm:gap-6">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-3 text-xs sm:text-sm">
                      <span className="px-3 py-1 rounded-full bg-slate-900/70 border border-slate-700/60 text-slate-100">ชั้น ป.{grade.replace('p', '')}</span>
